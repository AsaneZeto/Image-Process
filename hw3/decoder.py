...
def JPEG_decoder(path, image_height, image_width, output_path):

    dc, ac, tables, blocks_count = read_image_file(path)
    ...
    blocks_per_row = image_width // block_side
    blocks_per_col = image_height // block_side

    npmat = np.empty((image_height, image_width, 3), dtype=np.uint8)
    #print('npmat shape:', npmat.shape)

    for block_index in range(blocks_count):
        i = block_index // blocks_per_row * block_side
        j = block_index % blocks_per_row * block_side

        for c in range(3):
            zigzag = [dc[block_index, c]] + list(ac[block_index, :, c])
            quant_matrix = zigzag_to_block(zigzag)
            dct_matrix = dequantize(quant_matrix, 'lum' if c == 0 else 'chrom')
            block = idct2(dct_matrix)
            npmat[i:i+8, j:j+8, c] = block + 128

    #image = Image.fromarray(npmat, 'YCbCr')
    #image = image.convert('RGB')
    image = cv2.cvtColor(npmat, cv2.COLOR_YCrCb2RGB)
    #image.show()
    plt.imshow(image)
